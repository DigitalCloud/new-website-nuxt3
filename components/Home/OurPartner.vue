            ...
            >
              <swiper-slide v-for="(item, index) in partnerLogo" :key="index">
                <button
                  ...
                  class="bg-gray-300 p-font text-lg font-bold px-5 py-2 rounded-md flex justify-center items-center h-[60px] lg:h-[50px] partner-btn sm:h-[40px] sm:px-3"
                  @click="activePartnerLogo = item.id"
                >
                  <nuxt-img
                    :src="item.img"
                    alt="partner Logo"
                    class="max-h-[90%] max-w-[8rem]"
                  />
                </button>
              </swiper-slide>
            </swiper>
...
export default {
  ...
  computed: {
    activePartner() {
      return (
        this.partnerData?.partner?.find(
          (item) => item.id === this.activePartnerLogo
        ) || {}
      );
    },
  },
